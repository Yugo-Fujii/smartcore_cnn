...
fn read_csv_with_schema<P: AsRef<Path>>(path: P) -> PolarResult<DataFrame> {
    ...
    schema.with_column("species".to_string(), DataType::Utf8);
    schema.with_column("island".to_string(), DataType::Utf8);
    schema.with_column("culmen_length_mm".to_string(), DataType::Float64);
    schema.with_column("culmen_depth_mm".to_string(), DataType::Float64);
    schema.with_column("flipper_length_mm".to_string(), DataType::Float64);
    schema.with_column("body_mass_g".to_string(), DataType::Float64);
    schema.with_column("sex".to_string(), DataType::Utf8);

    let file = File::open(path).expect("Cannot open file.");
    CsvReader::new(file)
        .with_dtypes(Some(&schema))  // with_dtypesメソッドへ変更
        .has_header(true)
...
fn get_feature_target(df: &DataFrame) -> (PolarResult<DataFrame>, PolarResult<DataFrame>) {
    ...
    let target = df.select(["species"]); // [ ] を追加
    (features, target)
}
